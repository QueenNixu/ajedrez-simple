...
public class BlackPawn0 extends MovingObject {
	...
	public void update() {
		...
		if(Mouse.bp0 && Mouse.mousePressed) {
			...
			Cell.calcAllowedCells(originalPos, Constants.bp0Id, firstMove, gameState);
			...
			if(Mouse.lastPiece == Constants.bp0Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.bp0Id, Constants.BLACKSTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.bp0Id;
					...
					gameState.piecePosition[Constants.bp0Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
				...
				Vector2D originalPos = Mouse.originalPos;
				Cell.deAllowCells(originalPos, Constants.bp0Id, firstMove, gameState);
				if(valida && firstMove) {
