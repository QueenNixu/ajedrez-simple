...
public class BlackPawn0 extends GameObject {

	public BlackPawn0(Vector2D posicion, BufferedImage textura) {
	...
	public void update() {
		...
		if(Mouse.bp0 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == 24 && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !allyCell(newX, newY) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					...
					ObjectPosition.posicionesDelTablero[newX][newY] = 24;
					posicion.setX(getFromCell(Mouse.mouseXOnApp));
					posicion.setY(getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[24] = new Vector2D(newX*60, newY*60);
