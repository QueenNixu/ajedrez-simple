...
public class BlackPawn1 extends MovingObject {
	...
	public void update() {
		...
		if(Mouse.bp1 && Mouse.mousePressed) {
			...
			Cell.calcAllowedCells(originalPos, Constants.bp1Id, firstMove, gameState);
			//System.out.println("uwu -1");
			
		} else {
			if(Mouse.lastPiece == Constants.bp1Id && Mouse.mouseRealesed) {
				boolean valida;
				int newX = Cell.getZ(Mouse.mouseXOnApp);
				int newY = Cell.getZ(Mouse.mouseYOnApp);
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.bp1Id, Constants.BLACKSTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.bp1Id;
					...
					gameState.piecePosition[Constants.bp1Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
				...
				Vector2D originalPos = Mouse.originalPos;
				Cell.deAllowCells(originalPos, Constants.bp1Id, firstMove, gameState);
				if(valida && firstMove) {
