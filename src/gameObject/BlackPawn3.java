	...
	public void update() {
		...
		if(Mouse.bp3 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.bp3Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.bp3Id, Constants.BLACKSTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.bp3Id;
					...
					gameState.piecePosition[Constants.bp3Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
