...
public class BlackTowerLeft extends MovingObject {
	...
	public void update() {
		...
		if(Mouse.btl && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.btlId && Mouse.mouseRealesed) {
				boolean valida;
				int newX = Cell.getZ(Mouse.mouseXOnApp);
				int newY = Cell.getZ(Mouse.mouseYOnApp);
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.btlId, Constants.BLACKSTART) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					...
					ObjectPosition.posicionesDelTablero[newX][newY] = Constants.btlId;
					posicion.setX(Cell.getFromCell(Mouse.mouseXOnApp));
					posicion.setY(Cell.getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[Constants.btlId] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
