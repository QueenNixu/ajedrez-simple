...
public class BlackTowerLeft extends GameObject {
	...
	public void update() {
		...
		if(Mouse.btl && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == 20 && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !allyCell(newX, newY) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					...
					ObjectPosition.posicionesDelTablero[newX][newY] = 20;
					posicion.setX(getFromCell(Mouse.mouseXOnApp));
					posicion.setY(getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[20] = new Vector2D(newX*60, newY*60);
