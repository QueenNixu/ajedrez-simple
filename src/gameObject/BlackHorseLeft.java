...
public class BlackHorseLeft extends GameObject {

	public BlackHorseLeft(Vector2D posicion, BufferedImage textura) {
	...
	public void update() {
		...
		if(Mouse.bhl && Mouse.mousePressed) {
			...
			Cell.calcAllowedCells(originalPos, Constants.bhlId);
			...
			if(Mouse.lastPiece == Constants.bhlId && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.bhlId, Constants.BLACKSTART) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					System.out.println("Valida");
					ObjectPosition.posicionesDelTablero[Cell.getZ((int)Mouse.oriPosX)][Cell.getZ((int)Mouse.oriPosY)] = -1;
					ObjectPosition.posicionesDelTablero[newX][newY] = Constants.bhlId;
					posicion.setX(Cell.getFromCell(Mouse.mouseXOnApp));
					posicion.setY(Cell.getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[Constants.bhlId] = new Vector2D(newX*60, newY*60);
				...
				Vector2D originalPos = Mouse.originalPos;
				Cell.deAllowCells(originalPos, Constants.bhlId);
			}
		}
