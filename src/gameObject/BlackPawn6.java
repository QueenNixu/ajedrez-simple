	...
	public void update() {
		...
		if(Mouse.bp6 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.bp6Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.bp6Id, Constants.BLACKSTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.bp6Id;
					...
					gameState.piecePosition[Constants.bp6Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
