...
public class BlackBishopLeft extends GameObject {

	public BlackBishopLeft(Vector2D posicion, BufferedImage textura) {
...
public void update() {
		...
		if(Mouse.bbl && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == 18 && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !allyCell(newX, newY) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					...
					ObjectPosition.posicionesDelTablero[newX][newY] = 18;
					posicion.setX(getFromCell(Mouse.mouseXOnApp));
					posicion.setY(getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[18] = new Vector2D(newX*60, newY*60);
	...
	private void deallowCells(Vector2D originalPos) {
		...
		while(j-1 >= 0 && i-1 >= 0 && !allyCell(i-1,j-1, 18)) {
		...
		i = iAux;
		j = jAux;
		while(i+1 <= 7 && j-1 >= 0&& !allyCell(i+1,j-1, 18)) {
			System.out.println("PINTANDO: "+(i+1)+","+(j-1));
			ObjectPosition.allowedCellsBool[i+1][j-1] = false;
			i++;
			j--;
		...
		i = iAux;
		j = jAux;
		while(j+1 <= 7 && i+1 <= 7 && !allyCell(i+1,j+1, 18)) {
			System.out.println("PINTANDO: "+(i+1)+","+(j+1));
			ObjectPosition.allowedCellsBool[i+1][j+1] = false;
			j++;
			i++;
		...
		i = iAux;
		j = jAux;
		while(i-1 >= 0 && j+1 <= 7 && !allyCell(i-1,j+1, 18)) {
			System.out.println("PINTANDO: "+(i-1)+","+(j+1));
			ObjectPosition.allowedCellsBool[i-1][j+1] = false;
			i--;
			j++;
