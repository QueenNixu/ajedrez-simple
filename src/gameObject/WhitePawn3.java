	...
	public void update() {
		...
		if(Mouse.wp3 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.wp3Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wp3Id, Constants.WHITESTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.wp3Id;
					...
					gameState.piecePosition[Constants.wp3Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
