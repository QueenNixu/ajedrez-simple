	...
	public void update() {
		...
		if(Mouse.wp5 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.wp5Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wp5Id, Constants.WHITESTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.wp5Id;
					...
					gameState.piecePosition[Constants.wp5Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
