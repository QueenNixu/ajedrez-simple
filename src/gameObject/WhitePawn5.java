...
public class WhitePawn5 extends MovingObject {
	...
	public void update() {
		...
		if(Mouse.wp5 && Mouse.mousePressed) {
			...
			Cell.calcAllowedCells(originalPos, Constants.wp5Id, firstMove, gameState);
			...
			if(Mouse.lastPiece == Constants.wp5Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wp5Id, Constants.WHITESTART) && ObjectPosition.allowedCellsBool[newX][newY] ) {
				...
				Vector2D originalPos = Mouse.originalPos;
				Cell.deAllowCells(originalPos, Constants.wp5Id, firstMove, gameState);
				if(valida && firstMove) {
