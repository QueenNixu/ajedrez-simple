	...
	public void update() {
		...
		if(Mouse.wp1 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.wp1Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wp1Id, Constants.WHITESTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.wp1Id;
					...
					gameState.piecePosition[Constants.wp1Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
