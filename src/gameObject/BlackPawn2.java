...
public class BlackPawn2 extends GameObject {

	public BlackPawn2(Vector2D posicion, BufferedImage textura) {
	...
	public void update() {
		...
		if(Mouse.bp2 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == 26 && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !allyCell(newX, newY) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					...
					ObjectPosition.posicionesDelTablero[newX][newY] = 26;
					posicion.setX(getFromCell(Mouse.mouseXOnApp));
					posicion.setY(getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[26] = new Vector2D(newX*60, newY*60);
