...
public class BlackPawn2 extends MovingObject {
	...
	public void update() {
		...
		if(Mouse.bp2 && Mouse.mousePressed) {
			...
			Cell.calcAllowedCells(originalPos, Constants.bp2Id, firstMove, gameState);
			...
			if(Mouse.lastPiece == Constants.bp2Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.bp2Id, Constants.BLACKSTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.bp2Id;
					...
					gameState.piecePosition[Constants.bp2Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
				...
				Vector2D originalPos = Mouse.originalPos;
				Cell.deAllowCells(originalPos, Constants.bp2Id, firstMove, gameState);
				if(valida && firstMove) {
