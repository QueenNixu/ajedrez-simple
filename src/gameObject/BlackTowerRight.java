...
public class BlackTowerRight extends MovingObject {
	...
	public void update() {
		...
		if(Mouse.btr && Mouse.mousePressed && gameState.blackTurn) {
			...
			Cell.calcAllowedCells(originalPos, Constants.btrId, firstMove, gameState);
			//System.out.println("uwu -1");
			
		} else {
			if(Mouse.lastPiece == Constants.btrId && Mouse.mouseRealesed && gameState.blackTurn) {
				boolean valida;
				...
				Vector2D originalPos = Mouse.originalPos;
				Cell.deAllowCells(originalPos, Constants.btrId, firstMove, gameState);
				if(valida) {
					gameState.nextTurn();
					if(firstMove) {
