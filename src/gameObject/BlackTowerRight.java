...
public class BlackTowerRight extends GameObject {

	public BlackTowerRight(Vector2D posicion, BufferedImage textura) {
...
public void update() {
		...
		if(Mouse.btr && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == 23 && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !allyCell(newX, newY) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					...
					ObjectPosition.posicionesDelTablero[newX][newY] = 23;
					posicion.setX(getFromCell(Mouse.mouseXOnApp));
					posicion.setY(getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[23] = new Vector2D(newX*60, newY*60);
