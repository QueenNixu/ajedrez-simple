...
public class WhiteBishopLeft extends MovingObject {
	...
	public void update() {
		...
		if(Mouse.wbl && Mouse.mousePressed && gameState.whiteTurn) {
			...
			Cell.calcAllowedCells(originalPos, Constants.wblId, false, gameState);
			//System.out.println("uwu -1");
			
		} else {
			if(Mouse.lastPiece == Constants.wblId && Mouse.mouseRealesed && gameState.whiteTurn) {
				int newX = Cell.getZ(Mouse.mouseXOnApp);
				int newY = Cell.getZ(Mouse.mouseYOnApp);
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wblId, Constants.WHITESTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.wblId;
					posicion.setX(Cell.getFromCell(Mouse.mouseXOnApp));
					posicion.setY(Cell.getFromCell(Mouse.mouseYOnApp));
					gameState.piecePosition[Constants.wblId] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
					gameState.nextTurn();
				} else {
				...
				Vector2D originalPos = Mouse.originalPos;
				Cell.deAllowCells(originalPos, Constants.wblId, false, gameState);
			}
		}
