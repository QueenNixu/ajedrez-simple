	...
	public void update() {
		...
		if(Mouse.bp5 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.bp5Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.bp5Id, Constants.BLACKSTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.bp5Id;
					...
					gameState.piecePosition[Constants.bp5Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
