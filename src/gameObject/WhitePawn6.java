		...
		if(Mouse.wp6 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.wp6Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wp6Id, Constants.WHITESTART, null) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.wp6Id;
					...
					gameState.piecePosition[Constants.wp6Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
