...
public class BlackPawn4 extends GameObject {

	public BlackPawn4(Vector2D posicion, BufferedImage textura) {
	...
	public void update() {
		...
		if(Mouse.bp4 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == 28 && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !allyCell(newX, newY) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					...
					ObjectPosition.posicionesDelTablero[newX][newY] = 28;
					posicion.setX(getFromCell(Mouse.mouseXOnApp));
					posicion.setY(getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[28] = new Vector2D(newX*60, newY*60);
