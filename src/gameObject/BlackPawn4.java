	...
	public void update() {
		...
		if(Mouse.bp4 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.bp4Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.bp4Id, Constants.BLACKSTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.bp4Id;
					...
					gameState.piecePosition[Constants.bp4Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
