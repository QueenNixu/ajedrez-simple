...
public class BlackPawn7 extends MovingObject {
	...
	public void update() {
		...
		if(Mouse.bp7 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.bp7Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.bp7Id, Constants.BLACKSTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.bp7Id;
					...
					gameState.piecePosition[Constants.bp7Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
