...
public class BlackPawn7 extends GameObject {

	public BlackPawn7(Vector2D posicion, BufferedImage textura) {
	...
	public void update() {
		...
		if(Mouse.bp7 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == 31 && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !allyCell(newX, newY) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					...
					ObjectPosition.posicionesDelTablero[newX][newY] = 31;
					posicion.setX(getFromCell(Mouse.mouseXOnApp));
					posicion.setY(getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[31] = new Vector2D(newX*60, newY*60);
