...
public class WhitePawn7 extends MovingObject {
	...
	public void update() {
		...
		if(Mouse.wp7 && Mouse.mousePressed) {
			...
			Cell.calcAllowedCells(originalPos, Constants.wp7Id, firstMove, gameState);
			...
			if(Mouse.lastPiece == Constants.wp7Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wp7Id, Constants.WHITESTART) && ObjectPosition.allowedCellsBool[newX][newY] ) {
				...
				Vector2D originalPos = Mouse.originalPos;
				Cell.deAllowCells(originalPos, Constants.wp7Id, firstMove, gameState);
				if(valida && firstMove) {
