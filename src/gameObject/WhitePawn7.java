	...
	public void update() {
		...
		if(Mouse.wp7 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.wp7Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wp7Id, Constants.WHITESTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.wp7Id;
					...
					gameState.piecePosition[Constants.wp7Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
