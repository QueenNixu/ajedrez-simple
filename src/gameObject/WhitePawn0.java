...
public class WhitePawn0 extends GameObject {
	...
	public void update() {
		...
		if(Mouse.wp0 && Mouse.mousePressed) {
			...
			Cell.calcAllowedCells(originalPos, Constants.wp0Id);
			...
			if(Mouse.lastPiece == Constants.wp0Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wp0Id, Constants.BLACKSTART) && ObjectPosition.allowedCellsBool[newX][newY] ) {
				...
				Vector2D originalPos = Mouse.originalPos;
				Cell.deAllowCells(originalPos, Constants.wp0Id);
			}
		}
