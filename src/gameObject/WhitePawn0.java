	...
	public void update() {
		...
		if(Mouse.wp0 && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.wp0Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wp0Id, Constants.WHITESTART, gameState) && gameState.allowedCellsBool[newX][newY] ) {
					...
					gameState.posicionesDelTablero[newX][newY] = Constants.wp0Id;
					...
					gameState.piecePosition[Constants.wp0Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
