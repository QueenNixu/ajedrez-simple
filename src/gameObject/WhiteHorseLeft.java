...
public class WhiteHorseLeft extends GameObject {

	public WhiteHorseLeft(Vector2D posicion, BufferedImage textura) {
	...
	public void update() {
		...
		if(Mouse.whl && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == Constants.whlId && Mouse.mouseRealesed) {
				int newX = Cell.getZ(Mouse.mouseXOnApp);
				int newY = Cell.getZ(Mouse.mouseYOnApp);
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.whlId, Constants.WHITESTART) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					System.out.println("Valida");
					ObjectPosition.posicionesDelTablero[Cell.getZ((int)Mouse.oriPosX)][Cell.getZ((int)Mouse.oriPosY)] = -1;
					ObjectPosition.posicionesDelTablero[newX][newY] = Constants.whlId;
					posicion.setX(Cell.getFromCell(Mouse.mouseXOnApp));
					posicion.setY(Cell.getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[Constants.whlId] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
