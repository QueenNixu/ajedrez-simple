...
public class WhitePawn4 extends MovingObject {
	...
	public void update() {
		...
		if(Mouse.wp4 && Mouse.mousePressed) {
			...
			Cell.calcAllowedCells(originalPos, Constants.wp4Id, firstMove, gameState);
			...
			if(Mouse.lastPiece == Constants.wp4Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wp4Id, Constants.WHITESTART) && ObjectPosition.allowedCellsBool[newX][newY] ) {
				...
				Vector2D originalPos = Mouse.originalPos;
				Cell.deAllowCells(originalPos, Constants.wp4Id, firstMove, gameState);
				if(valida && firstMove) {
