...
public class WhiteTowerRight extends GameObject {

	public WhiteTowerRight(Vector2D posicion, BufferedImage textura) {
...
public void update() {
		...
		if(Mouse.wtr && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == 7 && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !allyCell(newX, newY) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					...
					ObjectPosition.posicionesDelTablero[newX][newY] = 7;
					posicion.setX(getFromCell(Mouse.mouseXOnApp));
					posicion.setY(getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[7] = new Vector2D(newX*60, newY*60);
