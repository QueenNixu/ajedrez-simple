...
public void update() {
		...
		if(Mouse.bq && Mouse.mousePressed) {
			...
			if(Mouse.lastPiece == 17 && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !allyCell(newX, newY) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					...
					ObjectPosition.posicionesDelTablero[newX][newY] = 17;
					posicion.setX(getFromCell(Mouse.mouseXOnApp));
					posicion.setY(getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[17] = new Vector2D(newX*60, newY*60);
	...
	private void deallowCells(Vector2D originalPos) {
		...
		while(j-1 >= 0 && i-1 >= 0 && !allyCell(i-1,j-1, 17)) {
		...
		i = iAux;
		j = jAux;
		while(i+1 <= 7 && j-1 >= 0&& !allyCell(i+1,j-1, 17)) {
			System.out.println("PINTANDO: "+(i+1)+","+(j-1));
			ObjectPosition.allowedCellsBool[i+1][j-1] = false;
			i++;
			j--;
		...
		i = iAux;
		j = jAux;
		while(j+1 <= 7 && i+1 <= 7 && !allyCell(i+1,j+1, 17)) {
			System.out.println("PINTANDO: "+(i+1)+","+(j+1));
			ObjectPosition.allowedCellsBool[i+1][j+1] = false;
			j++;
			i++;
		...
		i = iAux;
		j = jAux;
		while(i-1 >= 0 && j+1 <= 7 && !allyCell(i-1,j+1, 17)) {
			System.out.println("PINTANDO: "+(i-1)+","+(j+1));
			ObjectPosition.allowedCellsBool[i-1][j+1] = false;
			i--;
			j++;
	...
	private void calcAllowedCells(Vector2D originalPos) {
		...
		int i = iAux;
		int j = jAux;
		while(j-1 >= 0 && i-1 >= 0 && !allyCell(i-1,j-1)) {
			System.out.println("PINTANDO: "+(i-1)+","+(j-1));
		...
		i = iAux;
		j = jAux;
		while(i+1 <= 7 && j-1 >= 0&& !allyCell(i+1,j-1)) {
			System.out.println("PINTANDO: "+(i+1)+","+(j-1));
			ObjectPosition.allowedCellsBool[i+1][j-1] = true;
			i++;
			j--;
		...
		i = iAux;
		j = jAux;
		while(j+1 <= 7 && i+1 <= 7 && !allyCell(i+1,j+1)) {
			System.out.println("PINTANDO: "+(i+1)+","+(j+1));
			ObjectPosition.allowedCellsBool[i+1][j+1] = true;
			j++;
			i++;
		...
		i = iAux;
		j = jAux;
		while(i-1 >= 0 && j+1 <= 7 && !allyCell(i-1,j+1)) {
			System.out.println("PINTANDO: "+(i-1)+","+(j+1));
			ObjectPosition.allowedCellsBool[i-1][j+1] = true;
			i--;
			j++;
