...
public class WhitePawn2 extends GameObject {

	public WhitePawn2(Vector2D posicion, BufferedImage textura) {
	...
	public void update() {
		...
		if(Mouse.wp2 && Mouse.mousePressed) {
			...
			Cell.calcAllowedCells(originalPos, Constants.wp2Id);
			...
			if(Mouse.lastPiece == Constants.wp2Id && Mouse.mouseRealesed) {
				...
				if(newX >= 0 && newY >= 0 && !Cell.allyCell(newX, newY, Constants.wp2Id, Constants.BLACKSTART) && ObjectPosition.allowedCellsBool[newX][newY] ) {
					System.out.println("Valida");
					ObjectPosition.posicionesDelTablero[Cell.getZ((int)Mouse.oriPosX)][Cell.getZ((int)Mouse.oriPosY)] = -1;
					ObjectPosition.posicionesDelTablero[newX][newY] = Constants.wp2Id;
					posicion.setX(Cell.getFromCell(Mouse.mouseXOnApp));
					posicion.setY(Cell.getFromCell(Mouse.mouseYOnApp));
					ObjectPosition.piecePosition[Constants.wp2Id] = new Vector2D(newX*Constants.CELLSIZE, newY*Constants.CELLSIZE);
				...
				Vector2D originalPos = Mouse.originalPos;
				Cell.deAllowCells(originalPos, Constants.wp2Id);
			}
		}
